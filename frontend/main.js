...
// ----------------- Basic Function  ----------------- //
async function fetchBasics() { // Fetches the entire word list and the main word list
...
function generateTodaysWord() { // Generates the word for the day
  const now = new Date();
...
function setGameStats(stats = {}) { // Sets the game stats
  if (Object.keys(stats).length === 0) {
    if (!localStorage.getItem("tamilWordleStats")) { // If stats are not available, create a new one
      stats = {
        played: 0,
        wins: 0,
        streak: 0,
...
function setCurrentGameState(gameState, data_states) { // Sets the current game state
...
function freshDay() { // Removes all the previous data and starts a new game
...
function gameCompletedSetCurrentStats(condition) {
  ...
  if (intervalDur.hours > 0) {
    stats.lastWinTimeTaken = zero(intervalDur.hours) + ":" + zero(intervalDur.minutes) + ":" + zero(intervalDur.seconds);
  } else {
...
function createKeys() { // Creates tamil key for the keyboard and stores it in local storage
  let totalkeys = [];
  ...
  localStorage.setItem(
    "keyboard",
    JSON.stringify({ letters: shuffled, state: emptyStateKey })
  );

  return shuffled;
}

function createKeyboard(keyList) { // Dynamically creates the keyboard
  ...
  for (let i in keyList) {
    keys += `<button class="${keyState[i] != null ? keyState[i] + " key" : "key"
      }" data-key="${decodeURIComponent(keyList[i])}">${decodeURIComponent(
        keyList[i]
      )}</button>`;
    if (i == 9 || i == 18) {
...
async function shareButton() { // shares the result of the game
...
function setMode(mode) { // set dark or light mode
  if (mode === "dark") {
...
function switchMode() { // switch between dark and light mode
...
async function main() {
  ...
  if (localStorage.getItem("tamilWordle")) {
    const storage = JSON.parse(localStorage.getItem("tamilWordle"));
    if (storage.expires !== String(new Date()).slice(0, 15)) { // If the game is expired, start a new game
      freshDay();
      startGame();
    } else {
      setGameStats();
